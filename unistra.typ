...
//todo: add section and number as header, top left
...
#let cell(body, width: 100%, height: 100%) = rect(
  width: width,
  height: height,
  inset: 0mm,
  outset: 0mm,
...
#let gradientize(self, body, c1: blue-dark, c2: blue-dark) = {
  rect(fill: gradient.linear(c1, c2.lighten(20%), angle: 45deg), body)
}
...
#let hero(self: none, title: none, subtitle: none, img: none) = {
  let body = {
    align(center, image(img, width: 80%, height: auto))
  }
    ...
    block(
      ...
      stroke: (top: 0.5pt + self.colors.black),
      {
        ...
        grid(
          ...
          cell(text(self.info.title, weight: "bold") + " | " + self
            .info
            .subtitle + linebreak() + utils.call-or-display(
            self,
            footer-info-2,
          )),
